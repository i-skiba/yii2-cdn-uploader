var componentCdnUploader = (function() {
    ...
    var Component = {
        ...
        initialization : function () {
            $('.cdnuploader').not('.initialization').fileupload({
                add : function (e, data) {
                    ...
                    if(data.result.success && data.result.success.length > 0) {
                        var $wrapper = $self.closest('.cdn-upload-wrapper'),
                            files = CdnHelper.parseResponse(data.result.success),
                            info = files[0],
            ...
            // кроп изображения
            $('.cdn-upload-wrapper .uploader-crop-save-control').off('click');
            $('.cdn-upload-wrapper .uploader-crop-save-control').on('click', function () {
                var $wrapper = $(this).closest('.cdn-upload-wrapper'),
                    $uploader = $wrapper.find('.cdnuploader'),
                    options = Component.getPluginOptions($uploader),
            ...
            $('.cdn-upload-wrapper .uploader-crop-edit-control').off('click');
            $('.cdn-upload-wrapper .uploader-crop-edit-control').on('click', function (e) {
                var $wrapper = $(this).closest('.cdn-upload-wrapper'),
                    inputs = $wrapper.find('input[type=\'hidden\']'),
                    $uploader = $wrapper.find('.cdnuploader'),
